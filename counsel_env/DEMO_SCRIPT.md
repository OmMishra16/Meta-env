...
This is a compact theory-of-mind task for RL: the agent must reason over another actor's commitments under a budget. The published run-2 checkpoint is not oracle-level, but it beats the reward-hacking baselines by learning the trigger-then-evidence loop on held-out cases.
